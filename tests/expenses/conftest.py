"""Seeds the default system categories for expense tests.
...
(built straight from ORM metadata via `Base.metadata.create_all`). This
mirrors `tests/categories/conftest.py` so expense tests see the same
defaults a real deployment would.
"""
